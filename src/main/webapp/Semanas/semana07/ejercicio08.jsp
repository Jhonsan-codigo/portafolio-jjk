...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 08 - Extractor de Dominios | Jhon Contreras</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .url-item { background: rgba(0,0,0,0.2); border: 1px solid rgba(0,198,255,0.15); border-radius: 10px; padding: 12px; margin-bottom: 10px; }
        .url-original { color: #7dd3fc; font-size: 0.9rem; margin-bottom: 5px; word-break: break-all; }
        .url-dominio { color: #00c6ff; font-size: 1.1rem; font-weight: 700; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            <div class="numero">Ejercicio 08</div>
            <h1><i class="fas fa-globe"></i> Extractor de Dominios</h1>
            <p class="descripcion">Ingresa URLs completas y extrae solo el dominio raíz de cada una.</p>
        </div>
        <div class="metodos">
        ...
        <form action="calcular08" method="POST">
            <div class="form-group">
                <label for="urls"><i class="fas fa-link"></i> URLs (una por línea)</label>
                <textarea name="urls" id="urls" placeholder="https://www.google.com/search&#10;http://wikipedia.org&#10;https://github.com/user/repo" required></textarea>
            </div>
            <button type="submit" class="btn-calcular"><i class="fas fa-search"></i> Extraer Dominios</button>
        </form>
        <% if (request.getAttribute("error") != null) { %>
        <div class="error-msg" style="margin-top:20px"><i class="fas fa-exclamation-triangle"></i> <%= request.getAttribute("error") %></div>
        <% } %>
        <% if (request.getAttribute("dominios") != null) { %>
        <div class="resultados">
            <div class="result-card">
                <div class="label"><i class="fas fa-globe"></i> Dominios Extraídos</div>
                <%= request.getAttribute("dominios") %>
            </div>
        </div>
        <% } %>

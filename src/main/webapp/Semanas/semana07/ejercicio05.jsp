...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 05 - Analizador de Nombres | Jhon Contreras</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .result-card .original { color: #7dd3fc; font-size: 1.3rem; margin-bottom: 10px; }
        .result-card .flecha { color: #00c6ff; font-size: 1.5rem; margin: 10px 0; }
        .result-card .capitalizado { color: #fff; font-size: 1.8rem; font-weight: 700; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <h1><i class="fas fa-user-tag"></i> Analizador de Nombres</h1>
            <p class="descripcion">Ingresa un nombre completo en minúsculas y capitaliza cada palabra.</p>
        </div>
        <div class="metodos">
            <h3><i class="fas fa-code"></i> Métodos Utilizados</h3>
            <span class="tag">split()</span>
            <span class="tag">substring()</span>
            <span class="tag">toUpperCase()</span>
        </div>
        <form action="calcular05" method="POST">
            <div class="form-group">
                <label for="nombre"><i class="fas fa-user"></i> Nombre Completo (minúsculas)</label>
                <input type="text" name="nombre" id="nombre" placeholder="Ejemplo: juan perez gomez" required>
            </div>
            <button type="submit" class="btn-calcular"><i class="fas fa-magic"></i> Capitalizar</button>
        </form>
        <% if (request.getAttribute("error") != null) { %>
        <div class="error-msg" style="margin-top:20px"><i class="fas fa-exclamation-triangle"></i> <%= request.getAttribute("error") %></div>
        <% } %>
        <% if (request.getAttribute("capitalizado") != null) { %>
        <div class="resultados">
            <div class="result-card">
                <div class="label"><i class="fas fa-file-alt"></i> Nombre Original</div>
                <div class="original"><%= request.getAttribute("original") %></div>
                <div class="flecha"><i class="fas fa-arrow-down"></i></div>
                <div class="label"><i class="fas fa-user-check"></i> Nombre Capitalizado</div>
                <div class="capitalizado"><%= request.getAttribute("capitalizado") %></div>
            </div>
        </div>
        <% } %>

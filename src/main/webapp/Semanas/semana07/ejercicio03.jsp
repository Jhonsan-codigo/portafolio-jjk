...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 03 - Validador de Correos | Jhon Contreras</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .result-card .valor { color: #00c6ff; font-size: 1.5rem; font-weight: 700; }
        .valido { color: #22c55e !important; }
        .invalido { color: #ef4444 !important; }
        .detalle { color: #94a3b8; font-size: 0.9rem; margin-top: 10px; line-height: 1.8; }
        .detalle i { color: #00c6ff; margin-right: 8px; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <h1><i class="fas fa-envelope"></i> Validador de Correos</h1>
            <p class="descripcion">Verifica si una cadena es un correo electrónico válido básico.</p>
        </div>
        <div class="metodos">
            <h3><i class="fas fa-code"></i> Métodos Utilizados</h3>
            <span class="tag">contains()</span>
            <span class="tag">indexOf()</span>
            <span class="tag">lastIndexOf()</span>
            <span class="tag">endsWith()</span>
        </div>
        <form action="calcular03" method="POST">
            <div class="form-group">
                <label for="correo"><i class="fas fa-at"></i> Ingrese un correo electrónico</label>
                <input type="text" name="correo" id="correo" placeholder="Ejemplo: usuario@gmail.com" required>
            </div>
            <button type="submit" class="btn-calcular"><i class="fas fa-check-circle"></i> Validar Correo</button>
        </form>
        <% if (request.getAttribute("error") != null) { %>
        <div class="error-msg" style="margin-top:20px"><i class="fas fa-exclamation-triangle"></i> <%= request.getAttribute("error") %></div>
        <% } %>
        <% if (request.getAttribute("esValido") != null) { %>
        <div class="resultados">
            <div class="result-card">
                <div class="label"><i class="fas fa-envelope"></i> Correo Ingresado</div>
                <div class="valor"><%= request.getAttribute("correo") %></div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-clipboard-check"></i> Resultado</div>
                <div class="valor <%= request.getAttribute("esValido").equals("true") ? "valido" : "invalido" %>">
                    <i class="fas <%= request.getAttribute("esValido").equals("true") ? "fa-check-circle" : "fa-times-circle" %>"></i>
                    <%= request.getAttribute("esValido").equals("true") ? "CORREO VÁLIDO" : "CORREO INVÁLIDO" %>
                </div>
                <div class="detalle">

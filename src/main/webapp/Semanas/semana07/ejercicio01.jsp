...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 01 - Contador de Vocales | Jhon Contreras</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .vocales-grid { display: flex; justify-content: center; gap: 15px; flex-wrap: wrap; margin-top: 15px; }
        .vocal-item { background: rgba(0,198,255,0.1); border: 1px solid rgba(0,198,255,0.2); border-radius: 12px; padding: 15px 20px; text-align: center; min-width: 80px; }
        .vocal-item .letra { color: #00c6ff; font-size: 1.5rem; font-weight: 700; }
        .vocal-item .cantidad { color: #e2e8f0; font-size: 1.2rem; font-weight: 600; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            <div class="numero">Ejercicio 01</div>
            <h1><i class="fas fa-font"></i> Contador de Vocales</h1>
            <p class="descripcion">Ingresa una frase y cuenta cuántas vocales (a, e, i, o, u) contiene.</p>
        </div>
        <div class="metodos">
            <h3><i class="fas fa-code"></i> Métodos Utilizados</h3>
            <span class="tag">toLowerCase()</span>
            <span class="tag">charAt()</span>
        </div>
        <form action="calcular01" method="POST">
            <div class="form-group">
                <label for="frase"><i class="fas fa-keyboard"></i> Ingrese una frase</label>
                <textarea name="frase" id="frase" placeholder="Ejemplo: Hola Mundo, este es Java" required></textarea>
            </div>
            <button type="submit" class="btn-calcular"><i class="fas fa-calculator"></i> Contar Vocales</button>
        </form>
        <% if (request.getAttribute("error") != null) { %>
        <div class="error-msg" style="margin-top:20px"><i class="fas fa-exclamation-triangle"></i> <%= request.getAttribute("error") %></div>
        <% } %>
        <% if (request.getAttribute("totalVocales") != null) { %>
        <div class="resultados">
            <div class="result-card">
                <div class="label"><i class="fas fa-font"></i> Total de Vocales</div>
                <div class="valor"><%= request.getAttribute("totalVocales") %></div>
            </div>
            <div class="vocales-grid">
                <div class="vocal-item">
                    <div class="letra">A</div>
                    <div class="cantidad"><%= request.getAttribute("vocalA") %></div>
                </div>
                <div class="vocal-item">
                    <div class="letra">E</div>
                    <div class="cantidad"><%= request.getAttribute("vocalE") %></div>
                </div>
                <div class="vocal-item">
                    <div class="letra">I</div>
                    <div class="cantidad"><%= request.getAttribute("vocalI") %></div>
                </div>
                <div class="vocal-item">
                    <div class="letra">O</div>
                    <div class="cantidad"><%= request.getAttribute("vocalO") %></div>
                </div>
                <div class="vocal-item">
                    <div class="letra">U</div>
                    <div class="cantidad"><%= request.getAttribute("vocalU") %></div>
                </div>
            </div>
        </div>
        <% } %>

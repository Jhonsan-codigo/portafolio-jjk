...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 06 - Stock Aleatorio | Jhon Contreras</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .stocks-texto { color: #7dd3fc; font-size: 0.9rem; line-height: 1.8; word-break: break-all; max-height: 150px; overflow-y: auto; padding: 10px; background: rgba(0,0,0,0.2); border-radius: 8px; }
        .rangos-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            <div class="numero">Ejercicio 06</div>
            <h1><i class="fas fa-boxes"></i> Stock Aleatorio</h1>
            <p class="descripcion">Genera stock aleatorio para N productos (valores 0-200) y clasifica por rangos.</p>
        </div>
        <div class="formulas">
            <h3><i class="fas fa-chart-bar"></i> Rangos</h3>
            <div class="formula">a) Stock < 50</div>
            <div class="formula">b) 50 <= Stock < 100</div>
            <div class="formula">c) 100 <= Stock < 150</div>
            <div class="formula">d) Stock >= 150</div>
        </div>
        <form action="calcular06" method="POST">
            <div class="form-group">
                <label for="cantidadProductos"><i class="fas fa-cubes"></i> Cantidad de productos</label>
                <input type="number" name="cantidadProductos" id="cantidadProductos" min="1" placeholder="Ejemplo: 100" required>
            </div>
            <button type="submit" class="btn-calcular"><i class="fas fa-calculator"></i> Generar con Java</button>
        </form>
        <% if (request.getAttribute("error") != null) { %>
        <div class="error-msg" style="margin-top:20px"><i class="fas fa-exclamation-triangle"></i> <%= request.getAttribute("error") %></div>
        <% } %>
        <% if (request.getAttribute("rangoA") != null) { %>
        <div class="resultados">
            <div class="result-card">
                <div class="label"><i class="fas fa-cubes"></i> Cantidad de Productos</div>
                <div class="valor"><%= request.getAttribute("cantidadProductos") %></div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-list"></i> Stocks Generados</div>
                <div class="stocks-texto"><%= request.getAttribute("listaStocks") %></div>
            </div>
            <div class="rangos-grid">
                <div class="result-card">
                    <div class="label"><i class="fas fa-arrow-down"></i> Stock < 50</div>
                    <div class="valor"><%= request.getAttribute("rangoA") %></div>
                    <div class="unidad">productos</div>
                </div>
                <div class="result-card">
                    <div class="label"><i class="fas fa-arrows-alt-h"></i> 50 - 99</div>
                    <div class="valor"><%= request.getAttribute("rangoB") %></div>
                    <div class="unidad">productos</div>
                </div>
                <div class="result-card">
                    <div class="label"><i class="fas fa-arrows-alt-h"></i> 100 - 149</div>
                    <div class="valor"><%= request.getAttribute("rangoC") %></div>
                    <div class="unidad">productos</div>
                </div>
                <div class="result-card">
                    <div class="label"><i class="fas fa-arrow-up"></i> Stock >= 150</div>
                    <div class="valor"><%= request.getAttribute("rangoD") %></div>
                    <div class="unidad">productos</div>
                </div>
            </div>
        </div>

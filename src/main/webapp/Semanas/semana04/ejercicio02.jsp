...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 02 - Horas Extras | Jhon Contreras</title>
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <h1><i class="fas fa-clock"></i> Horas Extras</h1>
            <p class="descripcion">Las horas que exceden de 40 son extras. Las primeras 8 extras se pagan al doble y el resto al triple de la tarifa normal.</p>
        </div>
        <div class="formulas">
            <h3><i class="fas fa-function"></i> Reglas</h3>
            <div class="formula">Horas <= 40: tarifa normal</div>
            <div class="formula">Extras <= 8: tarifa x 2</div>
            <div class="formula">Extras > 8: primeras 8 x 2, resto x 3</div>
        </div>
        <form action="calcular02" method="POST">
            <div class="form-group">
                <label for="horasTrabajadas"><i class="fas fa-hourglass-half"></i> Horas trabajadas en la semana</label>
                <input type="number" name="horasTrabajadas" id="horasTrabajadas" step="any" min="0" placeholder="Ejemplo: 52" required>
            </div>
            <div class="form-group">
                <label for="tarifaNormal"><i class="fas fa-coins"></i> Tarifa normal por hora (S/)</label>
                <input type="number" name="tarifaNormal" id="tarifaNormal" step="any" min="0" placeholder="Ejemplo: 15.00" required>
            </div>
        ...
        <% } %>
        <% if (request.getAttribute("pagoTotal") != null) { %>
        <div class="resultados">
            <div class="result-card">
                <div class="label"><i class="fas fa-clock"></i> Horas Trabajadas</div>
                <div class="valor"><%= request.getAttribute("horasTrabajadas") %></div>
                <div class="unidad">horas</div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-coins"></i> Tarifa Normal</div>
                <div class="valor">S/ <%= request.getAttribute("tarifaNormal") %></div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-plus-circle"></i> Horas Extras</div>
                <div class="valor"><%= request.getAttribute("horasExtras") %></div>
                <div class="unidad">horas</div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-money-bill"></i> Pago Horas Normales</div>
                <div class="valor">S/ <%= request.getAttribute("pagoHorasNormales") %></div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-hand-holding-usd"></i> Pago Horas Extras</div>
                <div class="valor">S/ <%= request.getAttribute("pagoHorasExtras") %></div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-wallet"></i> Pago Total</div>
                <div class="valor">S/ <%= request.getAttribute("pagoTotal") %></div>
            </div>
        </div>
        <% } %>

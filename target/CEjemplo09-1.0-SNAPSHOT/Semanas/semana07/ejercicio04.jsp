...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 04 - Enmascarador de Tarjetas | Jhon Contreras</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .form-group input { width: 100%; padding: 14px 18px; border: 2px solid rgba(0,198,255,0.2); border-radius: 12px; background: rgba(0,0,0,0.2); color: #fff; font-size: 1rem; transition: 0.3s; letter-spacing: 2px; }
        ...
        .tarjeta-original { color: #7dd3fc; font-size: 1.3rem; font-family: 'Courier New', monospace; letter-spacing: 3px; margin-bottom: 15px; }
        .tarjeta-mascara { color: #fff; font-size: 2rem; font-weight: 800; font-family: 'Courier New', monospace; letter-spacing: 4px; background: linear-gradient(135deg, #1e3a5f, #0f172a); padding: 20px; border-radius: 12px; border: 1px solid rgba(0,198,255,0.3); }
        .asteriscos { color: #475569; }
        .ultimos { color: #00c6ff; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <h1><i class="fas fa-credit-card"></i> Enmascarador de Tarjetas</h1>
            <p class="descripcion">Ingresa un número de tarjeta de 16 dígitos y muestra solo los últimos 4.</p>
        </div>
        <div class="metodos">
            <h3><i class="fas fa-code"></i> Métodos Utilizados</h3>
            <span class="tag">substring()</span>
            <span class="tag">repeat()</span>
        </div>
        <form action="calcular04" method="POST">
            <div class="form-group">
                <label for="tarjeta"><i class="fas fa-credit-card"></i> Número de Tarjeta (16 dígitos)</label>
                <input type="text" name="tarjeta" id="tarjeta" maxlength="16" placeholder="1234567890123456" required>
            </div>
            <button type="submit" class="btn-calcular"><i class="fas fa-mask"></i> Enmascarar</button>
        </form>
        <% if (request.getAttribute("error") != null) { %>
        <div class="error-msg" style="margin-top:20px"><i class="fas fa-exclamation-triangle"></i> <%= request.getAttribute("error") %></div>
        <% } %>
        <% if (request.getAttribute("mascara") != null) { %>
        <div class="resultados">
            <div class="result-card">
                <div class="label"><i class="fas fa-credit-card"></i> Número Original</div>
                <div class="tarjeta-original"><%= request.getAttribute("original") %></div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-mask"></i> Tarjeta Enmascarada</div>
                <div class="tarjeta-mascara">
                    <span class="asteriscos"><%= request.getAttribute("asteriscos") %></span><span class="ultimos"><%= request.getAttribute("ultimos4") %></span>
                </div>
            </div>
        </div>
        <% } %>

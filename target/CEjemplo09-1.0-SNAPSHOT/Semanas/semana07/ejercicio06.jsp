...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 06 - Detector de Palíndromos | Jhon Contreras</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .result-card .valor { font-size: 2rem; font-weight: 800; }
        .es-palindromo { color: #22c55e; }
        .no-palindromo { color: #ef4444; }
        .procesado { color: #7dd3fc; font-size: 1.2rem; margin-top: 10px; font-family: 'Courier New', monospace; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <h1><i class="fas fa-sync-alt"></i> Detector de Palíndromos</h1>
            <p class="descripcion">Ingresa una frase y verifica si se lee igual de izquierda a derecha.</p>
        </div>
        <div class="metodos">
        ...
        <form action="calcular06" method="POST">
            <div class="form-group">
                <label for="frase"><i class="fas fa-keyboard"></i> Ingrese una frase o palabra</label>
                <input type="text" name="frase" id="frase" placeholder="Ejemplo: reconocer" required>
            </div>
            <button type="submit" class="btn-calcular"><i class="fas fa-search"></i> Verificar Palíndromo</button>
        </form>
        <% if (request.getAttribute("error") != null) { %>
        <div class="error-msg" style="margin-top:20px"><i class="fas fa-exclamation-triangle"></i> <%= request.getAttribute("error") %></div>
        <% } %>
        <% if (request.getAttribute("esPalindromo") != null) { %>
        <div class="resultados">
            <div class="result-card">
                <div class="label"><i class="fas fa-file-alt"></i> Texto Procesado</div>
                <div class="procesado"><%= request.getAttribute("procesado") %></div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-clipboard-check"></i> Resultado</div>
                <div class="valor <%= request.getAttribute("esPalindromo").equals("true") ? "es-palindromo" : "no-palindromo" %>">
                    <i class="fas <%= request.getAttribute("esPalindromo").equals("true") ? "fa-check-circle" : "fa-times-circle" %>"></i>
                    <%= request.getAttribute("esPalindromo").equals("true") ? "¡ES PALÍNDROMO!" : "NO ES PALÍNDROMO" %>
                </div>
            </div>
        </div>
        <% } %>

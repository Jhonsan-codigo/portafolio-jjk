...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 07 - Censor de Malas Palabras | Jhon Contreras</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .form-group textarea { width: 100%; padding: 14px 18px; border: 2px solid rgba(0,198,255,0.2); border-radius: 12px; background: rgba(0,0,0,0.2); color: #fff; font-size: 1rem; transition: 0.3s; resize: vertical; min-height: 80px; }
        ...
        .texto-original { color: #7dd3fc; font-size: 1rem; line-height: 1.6; padding: 15px; background: rgba(0,0,0,0.2); border-radius: 10px; margin-bottom: 10px; }
        .texto-censurado { color: #fff; font-size: 1rem; line-height: 1.6; padding: 15px; background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.2); border-radius: 10px; }
        .censurada { color: #ef4444; font-weight: 700; background: rgba(239,68,68,0.2); padding: 2px 6px; border-radius: 4px; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            <div class="numero">Ejercicio 07</div>
            <h1><i class="fas fa-comment-slash"></i> Censor de Malas Palabras</h1>
            <p class="descripcion">Ingresa un mensaje y una lista de palabras prohibidas para censurarlas con asteriscos.</p>
        </div>
        <div class="metodos">
            <h3><i class="fas fa-code"></i> Métodos Utilizados</h3>
            <span class="tag">replaceAll()</span>
        </div>
        <form action="calcular07" method="POST">
            <div class="form-group">
                <label for="mensaje"><i class="fas fa-comment"></i> Mensaje del Usuario</label>
                <textarea name="mensaje" id="mensaje" placeholder="Escribe tu mensaje aquí..." required></textarea>
            </div>
            <div class="form-group">
                <label for="prohibidas"><i class="fas fa-ban"></i> Palabras Prohibidas (separadas por coma)</label>
                <input type="text" name="prohibidas" id="prohibidas" placeholder="Ejemplo: tonto,feo,malo" required>
            </div>
            <button type="submit" class="btn-calcular"><i class="fas fa-shield-alt"></i> Censurar Mensaje</button>
        </form>
        <% if (request.getAttribute("error") != null) { %>
        <div class="error-msg" style="margin-top:20px"><i class="fas fa-exclamation-triangle"></i> <%= request.getAttribute("error") %></div>
        <% } %>
        <% if (request.getAttribute("mensajeCensurado") != null) { %>
        <div class="resultados">
            <div class="result-card">
                <div class="label"><i class="fas fa-file-alt"></i> Mensaje Original</div>
                <div class="texto-original"><%= request.getAttribute("mensajeOriginal") %></div>
            </div>
            <div class="result-card">
                <div class="label"><i class="fas fa-shield-alt"></i> Mensaje Censurado</div>
                <div class="texto-censurado"><%= request.getAttribute("mensajeCensurado") %></div>
            </div>
        </div>
        <% } %>

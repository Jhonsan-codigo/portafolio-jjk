...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 09 - Reparto de Dinero | Jhon Contreras</title>
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <h1><i class="fas fa-hand-holding-usd"></i> Reparto de Dinero</h1>
            <p class="descripcion">Un padre repartirá una cantidad de dinero entre sus cinco hijos con porcentajes diferentes.</p>
        </div>
        <div class="formulas">
            <h3><i class="fas fa-function"></i> Fórmulas / Datos</h3>
            Tamar: 85% de Josué | Josué: 27% del total | Caleb: 23% de (Josué+Daniel) | Daniel: 25% del total | David: el resto
        </div>
        <form action="calcular09" method="POST">
            <div class="form-group"><label for="monto"><i class="fas fa-coins"></i> Monto total a repartir (S/)</label><input type="number" name="monto" id="monto" step="any" min="0" placeholder="Ejemplo: 10000" required></div>
        ...
        %>
        <% String tamar = (String) request.getAttribute("tamar"); if (tamar != null) { String jose = (String) request.getAttribute("jose"); String caleb = (String) request.getAttribute("caleb"); String daniel = (String) request.getAttribute("daniel"); String david = (String) request.getAttribute("david"); %><div class="resultados"><div class="result-card"><div class="label">Tamar (85% de Josué)</div><div class="valor">S/ <%= tamar %></div></div><div class="result-card"><div class="label">Josué (27% del total)</div><div class="valor">S/ <%= jose %></div></div><div class="result-card"><div class="label">Caleb (23% de Josué+Daniel)</div><div class="valor">S/ <%= caleb %></div></div><div class="result-card"><div class="label">Daniel (25% del total)</div><div class="valor">S/ <%= daniel %></div></div><div class="result-card"><div class="label">David (Resto)</div><div class="valor">S/ <%= david %></div></div></div><% } %>

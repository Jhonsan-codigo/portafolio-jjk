...
<head>
    <meta charset="UTF-8">
    <title>Ejercicio 08 - Conversión de Tiempo | Jhon Contreras</title>
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <h1><i class="fas fa-clock"></i> Conversión de Tiempo</h1>
            <p class="descripcion">Crear un programa que permita convertir una cantidad de segundos en horas, minutos y segundos.</p>
        </div>
        <div class="formulas">
            <h3><i class="fas fa-function"></i> Fórmulas / Datos</h3>
            1 hora = 3600 seg | 1 min = 60 seg
        </div>
        <form action="calcular08" method="POST">
            <div class="form-group"><label for="segundos"><i class="fas fa-stopwatch"></i> Segundos totales</label><input type="number" name="segundos" id="segundos" step="1" min="0" placeholder="Ejemplo: 3661" required></div>
        ...
        %>
        <% Integer horas = (Integer) request.getAttribute("horas"); Integer minutos = (Integer) request.getAttribute("minutos"); Integer segundos = (Integer) request.getAttribute("segundos"); if (horas != null) { %><div class="resultados"><div class="result-card"><div class="label"><i class="fas fa-clock"></i> Resultado</div><div class="valor"><%= horas %>h <%= minutos %>m <%= segundos %>s</div></div></div><% } %>
